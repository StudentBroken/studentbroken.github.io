...
function renderWidgets(etapeKey) {
    ...
    ['etape1', 'etape2', 'etape3'].forEach(etape => {
        (mbsData[etape] || []).forEach(subject => {
            if (!allSubjectsAcrossEtapes.has(subject.code)) allSubjectsAcrossEtapes.set(subject.code, { ...subject, competencies: [] });
        });
        (mbsData[etape] || []).forEach(subject => {
...
function openExpandedView(subject) {
    ...
    summaryWidget.innerHTML = `
        <div class="widget-top-section" style="flex-basis: 33.3%; flex-shrink: 0;">
            <div class="widget-info">
        ...
        <div class="histogram-container" style="flex-basis: 33.3%; min-height: 0;"><canvas id="expanded-hist-chart"></canvas></div>
        <div class="histogram-container" style="flex-basis: 33.3%; min-height: 0;"><canvas id="expanded-line-chart"></canvas></div>`;
    ...
    detailsWidget.innerHTML = `
        ...
        <div class="details-widget-body" style="display: flex; flex-direction: column; flex-grow: 1; min-height: 0;">
            <div class="competency-widgets" style="flex-shrink: 0;"></div>
            <div class="graph-container" style="position: relative; flex-grow: 1; min-height: 150px; margin: 15px 0;"><canvas id="assignmentsChart"></canvas></div>
            <div class="calculator-container" style="flex-shrink: 0;"></div>
            <div class="details-footer" style="margin-top: auto; padding-top: 15px; flex-shrink: 0;">
                <a href="projection.html?subject=${encodeURIComponent(subject.name)}" class="btn-secondary" style="display: block; text-align: center;">Plus d'information</a>
            </div>
        </div>`;
...
function openExpandedViewForGeneral(subjects, etapeKey, average, history) {
    ...
    detailsWidget.innerHTML = `
        ...
        <div class="details-widget-body" style="display: flex; flex-direction: column; flex-grow: 1; min-height: 0;">
            <div class="competency-widgets" style="flex-shrink: 0; display: flex; flex-wrap: wrap; gap: 10px;"></div>
            <div class="graph-container" style="position: relative; flex-grow: 1; min-height: 150px; margin: 15px 0;"><canvas id="assignmentsChart"></canvas></div>
            <div class="calculator-container" style="flex-shrink: 0;">
                <p>Le planificateur est disponible uniquement pour les matières individuelles.</p>
            </div>
             <div class="details-footer" style="margin-top: auto; padding-top: 15px; flex-shrink: 0;">
                <a href="projection.html" class="btn-secondary" style="display: block; text-align: center;">Plus d'information</a>
            </div>
        </div>`;
...
function populateDetailsWidget(widget, subject) {
    ...
    if (compsForChart.length > 0 && compWidgets.length > 0) compWidgets[0].click();
    else widget.querySelector('.graph-container').style.display = 'none';
...
async function fetchRankingData() {
    ...
    try {
        ...
        // --- GHOST LOADING FIX --- Trigger event on completion
        document.dispatchEvent(new CustomEvent('rankingDataLoaded'));
    }
}
...
function populateRankingWidget(widget, rankingKey) {
    ...
    // --- GHOST LOADING FIX --- Check status and add event listener
    if (rankingData.status === 'loading') {
        ...
        const onDataLoaded = () => {
            populateRankingWidget(widget, rankingKey); // Re-run self
            document.removeEventListener('rankingDataLoaded', onDataLoaded); // Cleanup
        };
        document.addEventListener('rankingDataLoaded', onDataLoaded);
        return;
    }
    ...
    contentEl.innerHTML = `
        <div class="widget-rank" style="flex-shrink: 0;">${rank} sur ${total} <span style="margin-left: 8px;">(Top ${percentile}%)</span></div>
        ...
        <div class="histogram-container" style="flex-grow: 1; margin-top:20px; min-height: 0;"><canvas id="ranking-comparison-chart"></canvas></div>`;
...
// ... All other functions (openOrderEditor, helpers, charts) remain the same.
// They are omitted here for brevity but are required in the final file.
...
function renderHistogram(canvasId, subject, chartStore = activeWidgetCharts) {
    ...
    if (widget && chartStore === activeWidgetCharts) {
        ...
        const orderBtn = widget.querySelector('.order-edit-btn');
...
function renderSubjectAveragesChart(subjects) {
    if (activeExpandedCharts['assignmentsChart']) activeExpandedCharts['assignmentsChart'].destroy();
    const ctx = document.getElementById('assignmentsChart').getContext('2d');
    activeExpandedCharts['assignmentsChart'] = new Chart(ctx, {
        type: 'bar',
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: subjects.map(s => (s.average < 60 ? 'rgba(231, 76, 60, 0.7)' : 'rgba(41, 128, 185, 0.7)')),
            }]
        },
        options: { responsive: true, maintainAspectRatio: false, scales: { y: { suggestedMin: 50, suggestedMax: 100, beginAtZero: false } }, plugins: { legend: { display: false }, title: { display: true, text: 'Moyennes par Matière' } } }
    });
}
