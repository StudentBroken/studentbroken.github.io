...
// --- NEW --- State to track the currently open ranking widget for auto-refresh
...
function openExpandedView(subject) {
    ...
    // --- MODIFIED --- Applying flex styles for 1/3 layout
    ...
    summaryWidget.innerHTML = `
        <div class="widget-top-section" style="flex: 1;"><div class="widget-info"><h3 class="widget-title">${subject.name} (Résumé)</h3><p class="widget-average">${subjectAverage.toFixed(2)}%</p></div><div class="gauge-container"><canvas id="expanded-gauge-chart"></canvas></div></div>
        <div class="histogram-container" style="flex: 1; margin-top: 10px;"><canvas id="expanded-hist-chart"></canvas></div>
        <div class="histogram-container" style="flex: 1; margin-top: 10px;"><canvas id="expanded-line-chart"></canvas></div>`;
    
    // --- MODIFIED --- Applying flex styles for layout and adding button
    ...
    detailsWidget.innerHTML = `
        ...
        <div class="details-widget-body" style="flex-grow: 1; min-height: 0;">
        ...
        <a href="projection.html?subject=${encodeURIComponent(subject.name)}" class="btn-secondary" style="margin-top: 20px; text-align: center; flex-shrink: 0;">Plus d'information</a>`;
    ...
    currentRankingInfo = { widget: rankingWidget, key: rankingKey }; // Store for auto-refresh
...
function openExpandedViewForGeneral(subjects, etapeKey, average, history) {
    ...
    summaryWidget.style.cssText = 'display: flex; flex-direction: column;';
    summaryWidget.innerHTML = `
        <div class="widget-top-section" style="flex: 1;"><div class="widget-info"><h3 class="widget-title">${title} (Résumé)</h3><p class="widget-average">${average.toFixed(2)}%</p></div><div class="gauge-container"><canvas id="expanded-gauge-chart"></canvas></div></div>
        <div class="histogram-container" style="flex: 1; margin-top: 10px;"><canvas id="expanded-hist-chart"></canvas></div>
        <div class="histogram-container" style="flex: 1; margin-top: 10px;"><canvas id="expanded-line-chart"></canvas></div>`;
    ...
    detailsWidget.innerHTML = `
        ...
        <a href="projection.html" class="btn-secondary" style="margin-top: 20px; text-align: center; flex-shrink: 0;">Plus d'information</a>`;
...
function closeExpandedView() {
    ...
    currentRankingInfo = { widget: null, key: null }; // Clear state on close
}
...
// --- MODIFIED --- Creates one big bubble instead of many small ones
function populateGeneralDetailsWidget(widget, subjects, average) {
    ...
    const summaryWidget = document.createElement('div');
    summaryWidget.className = 'comp-widget active'; // 'active' to give it a border
    summaryWidget.style.flexBasis = '100%'; // Make it take the full width
...
async function fetchRankingData() {
    ...
    try {
        ...
        if (currentRankingInfo.widget) { // Also update on error
             populateRankingWidget(currentRankingInfo.widget, currentRankingInfo.key);
        }
    }
}
...
function populateRankingWidget(widget, rankingKey) {
    ...
    // --- MODIFIED --- Added fixed height to leaderboard container
...
// ... (Rest of the functions: openOrderEditor, calculateAveragesFromRawData, getRank, setupGoalFramework, all chart functions are unchanged and required)
// --- OMITTED FOR BREVITY ---
